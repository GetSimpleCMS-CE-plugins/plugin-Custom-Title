...
class CustomTitlePlugin
{
    ...
    public function __construct()
    {
        ...
        register_plugin(
            ...
            [$this, 'renderSettingsPage'] // Function that displays content
        );

        // Initiat Hooks
        add_action('plugins-sidebar', 'createSideMenu', [$this->getPluginId(), 'Custom Title']);
        add_action('edit-extras', [$this, 'renderEditExtras']);
        add_action('changedata-save', [$this, 'saveCustomTitle']);

        add_filter('data_index', [$this, 'hookDataIndex']);
    }
    ...
    private function replaceTemplatePlaceholders($title)
    {
        ...
        $replace_mapping = ['%sitename%'   => get_site_name(false), '%pagetitle%'   => get_page_clean_title(false), '%menutitle%'   => $pdata->menu, '%parenttitle%' => $parent_title ?? ''];
    ...
    public function renderSettingsPage()
    {
        ...
        <form method="post" action="">
            <input type="text" name="default_title"  style="box-sizing:border-box;width:100%;padding:10px;" value="{$default_title}"/>
            <input type="submit" name="save" style="background:#000;color:#fff;padding:0.7rem 1rem;border:none; margin-top:10px;" value="Save" />
        </form>
        <br /><br />
        <h3>Information:</h3>
